...
with $n_D = 3.3 \times 10^{19}$~m${}^{-3}$ and $\tau_p \sim 30$~s,
$f_T^* \sim 0.5$ and $f_{{}^3\text{He}}^* \sim 0.1$, with the latter
smaller because $\langle\sigma v\rangle_{\text{D}{}^3\text{He}}$ is
~13$\times$ smaller than $\langle\sigma v\rangle_{\text{DT}}$ at this
temperature (see
\texttt{examples/dhe3\_burn\_fractions.py}). Pulsed-FRC architectures
with active inter-shot ${}^3$He recovery (Helion-style) reach the
...
of an external atom.
At the default $f_{\text{DD}} = 0.131$ (corresponding to a
50/50 D/${}^3$He mix at $T_i = 70$~keV with Bosch-Hale cross sections,
the textbook reference of Wesson and Santarius/Kulcinski), the
effective neutron energy fraction
...
\begin{itemize}
  \item Tank: \$3--15~M, central \$7~M.
    ...
    below that price.  ITER cryostat ($\sim\$30\text{--}40$/kg,
  ...
  \item Cryopumps: \$10--25~M, central \$15~M.
    ...
    and industrial cryopump pricing (Edwards/Leybold STP-iXR class,
