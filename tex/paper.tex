...
studies. \Cref{sec:tokamak-model} presents the 0D tokamak model.
...
because it directly determines economic viability ($Q_{\text{eng}} < 1$
...
Pulsed FRC  &  1.5$\times$ & 1.0 & --- & Theta-pinch formation coils \\
Theta pinch &  1.5$\times$ & 1.0 & --- & Compression coils \\
MagLIF      &  2.0$\times$ & 1.0 & --- & Axial field solenoid \\
Mag.\ target &  1.5$\times$ & 1.0 & --- & Guide-field solenoid \\
Plasma jet  &  1.5$\times$ & 1.0 & --- & Guide-field solenoid \\
Orbitron    &  1.5$\times$ & 1.0 & --- & Electrostatic confinement \\
Polywell    &  2.0$\times$ & 1.0 & --- & Polyhedral magrid \\
IFE/Z-pinch & \multicolumn{3}{l}{\$0 (no confinement magnets)} & --- \\
\bottomrule
\end{tabular}
\end{center}
...
For pulsed concepts, the account covers the primary driver capital ---
...
as electricity.  Inductive DEC (Helion-type pulsed FRC) is captured in
...
\begin{equation}
  ...
  \quad [\text{M\$}]
\end{equation}
where $c_{\text{dec}} = 140$~M\$ is the NOAK all-in 2024 USD reference
...
\begin{itemize}
  \item Tank and incremental vacuum: \$15--30~M, central \$20~M.
    Industrial-grade fabricated 304L stainless ($\$8\text{--}12$/kg
    cleaned and ASME~Section~VIII certified for $10^{-5}$ to
    $10^{-6}$~Torr service) at 1500--3000 tonnes.  ITER cryostat
    ($\sim \$35$/kg, nuclear-grade) and LIGO UHV ($10^{-9}$~Torr,
    overspec) bracket above; bare-plate steel pricing ($\$5\text{--}8$/kg)
    bounds below.
  ...
  \item Cryopumps: \$10--25~M, central \$15~M.
    ...
    \citep{iter_cryopumps} (8 units at $\sim \$6$M each, total
    $\sim \$50$M for 600,000~L/s via Fusion for Energy contracts) and
    industrial cryopump pricing (Edwards/Leybold STP-iXR class,
    $\$200\text{--}500$k per 20,000~L/s) bracket the figure.
  \item Heat collection panels: \$10--20~M, central \$15~M.
    Active-cooled tungsten panels for the 30--90~MW heat load
    deposited on grids and collectors (5--15\% of charged-particle
    power for geometrically transparent grids).  Scaled from ITER
    NBI beam-dump procurement \citep{iter_nbi_dump}
    ($\sim \$5\text{--}10$M per 16~MW beam line).  ITER divertor
    pricing ($\$60$M/GW${}_\text{th}$) is overspec, since VB grids
    are not exposed to direct fusion plasma or 14~MeV neutron flux.
...
The hardware subtotal is \$90--167~M, central \$122~M.  Adding 15\%
installation (NOAK) gives \$105--190~M total, central \$140~M, which
...
fission-level staffing (500 per GWe). Our staffing-based
...
total capital \citep{woodruff2026}; our D-T estimate of 8\% is
consistent with this, while our p-${}^{11}$B estimate of 5\%
