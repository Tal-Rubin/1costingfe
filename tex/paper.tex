...
\hspace{1em}22.01.03$^\dagger$ & Coils (magnets) & Conductor kAm $\times$ \$/kAm $\times$ markup \\
\hspace{1em}22.01.04 & Supplementary heating & Per-MW linear (NBI, ICRF, ECRH, LHCD) \\
\hspace{1em}22.01.05 & Primary structure & Volume $\times$ power scale \\
\hspace{1em}22.01.06 & Vacuum system & Volume $\times$ power scale \\
\hspace{1em}22.01.07 & Power supplies & Power-scaled (0.7) \\
\hspace{1em}22.01.08 & Divertor / target factory & Thermal-scaled (MFE) or power-scaled (IFE) \\
\hspace{1em}22.01.09 & Direct energy converter & Excluded (override for mirror/FRC) \\
...
23$^\dagger$ & Turbine plant equipment & Linear scaling on gross electric (NETL-calibrated) \\
...
26$^\dagger$ & Heat rejection systems & Linear scaling on gross electric (NETL-calibrated) \\
...
\midrule
D-T    & 528 & 607 & Enhanced industrial (tritium confinement, hot cell, rad-HVAC) \\
D-D    & 466 & 536 & Enhanced industrial (reduced tritium/activation scope) \\
D-${}^3$He & 344 & 396 & Light enhanced industrial \\
...
\item \textbf{Hot cell:} D-T \$104M (full remote handling, 3--4~ft
  ...
  reactor building (UKAEA STEP/PROCESS model), not a separate
  structure.
...
\item \textbf{Security:} Tritium is not listed in 10~CFR Part~37
  Category~1/2 table---no armed response requirements for any fusion
  fuel.  All fuels use standard industrial security (\$2--3.5M).

\item \textbf{Site improvements:} D-T \$115M (tritium monitoring
  perimeter); p-${}^{11}$B \$69M (standard industrial site).
\end{itemize}
...
\midrule
Gas CCGT & 150--250 & Industrial \\
Coal (NETL B12A) & 175--300 & Heavy industrial \\
Fusion pB11 (this model) & 308 & Industrial (1.5$\times$ CCGT) \\
Fusion D-T (this model) & 528 & Enhanced industrial (2.6$\times$ CCGT) \\
Fission nuclear (Part~50) & 800--1{,}300 & Nuclear-grade \\
\bottomrule
\end{tabular}
\end{center}

Source: from-scratch derivation using industrial construction
benchmarks (Cushman~\&~Wakefield 2025, NETL 2019, NREL ATB 2024,
Breakthrough Institute 2022).  Supersedes ARIES/pyFECONS values.
...
Total conductor quantity is estimated from the magnetic energy stored
in the coil system:
\begin{equation}
\label{eq:cas2203}
...
that is essentially identical to any large steam-Rankine thermal
plant.  These accounts are \textbf{fuel-independent}: the choice of fuel
affects the reactor island (CAS22) but not the turbine hall, switchyard,
or cooling towers.

Each account scales linearly with gross electric power:
\begin{equation}
\label{eq:bop}
C_{2x} = n_{\text{mod}} \times P_{\text{et}} \times c_{2x}
\end{equation}
\noindent where $P_{\text{et}}$ is gross electric output (MW) and $c_{2x}$
is a per-MW coefficient (M\$/MW, 2024\$).  \Cref{tab:bop} lists the adopted
values.

\begin{table}[htbp]
\caption{Balance of plant per-MW coefficients (2024\$).}
\label{tab:bop}
\centering
\begin{tabular}{llr}
\toprule
CAS & Scope & $c_{2x}$ (M\$/MW) \\
\midrule
23 & Steam TG, condenser, feedwater & 0.198 \\
24 & Switchyard, transformers, cabling & 0.084 \\
25 & Fire protection, compressed air, HVAC & 0.051 \\
26 & Cooling towers, circulating water & 0.034 \\
\midrule
\multicolumn{2}{l}{\textit{Total BOP}} & \textit{0.367} \\
\bottomrule
\end{tabular}
\end{table}

The coefficients derive from the ARIES cost-account tradition
...
(2019\$) values are escalated by a CPI factor of 1.22.  At 1~GWe net
(${\sim}$1.3~GW gross), total BOP cost is ${\sim}$\$477M
(${\sim}$\$367/kW~gross), consistent with industry benchmarks for steam
turbine-generator sets (Siemens, GE), switchyard equipment, and mechanical-draft
cooling towers.

Cross-checks against the NREL ATB 2024 nuclear cost breakdown (3.9\% of
total capital for energy conversion, 6.3\% for electrical equipment) confirm
that the adopted values are conservative---appropriate for NOAK fusion plants
...
The default assumes a PbLi blanket concept for D-T: ${\sim}$4{,}000 tonnes
of Pb-17Li eutectic at ${\sim}$\$3/kg plus enriched lithium premium,
totaling \$15M at 1~GWe.  This is small relative to CAS22
(${\sim}$\$2.7B) because PbLi is an inexpensive commodity (mostly lead).

\textbf{Design-dependent override:} The Helium-Cooled Pebble Bed (HCPB)
blanket concept requires ${\sim}$300--490 tonnes of beryllium pebbles
...
operation. We decompose CAS50 into six sub-accounts, three of which
are fuel-dependent:
...
\item \textbf{C51 (Shipping/transportation):} 1.5\% of CAS20. World
  Nuclear Association reports transportation at $\sim$2\% of overnight
...
\item \textbf{C55 (Startup fuel/inventory):} The fusion analogue of
  the fission ``first core'' ($\sim$3\% of overnight cost,
  $\sim$\$120M for a 1~GWe PWR). D-T requires 1--2~kg of tritium at
  $\sim$\$30{,}000/g ($\sim$\$40M); p-${}^{11}$B requires only
...
We estimate total decommissioning cost by starting from the fission
reference and removing inapplicable cost drivers (spent fuel
$\sim$\$300/kWe, actinide waste $\sim$\$200/kWe, Part~50 overhead
$\sim$\$100/kWe, armed security $\sim$\$100/kWe), then adding
...
\subsubsection{Comparison with prior methods.}
The previous implementation used flat placeholder values
(\$1M shipping, \$0.5M insurance, \$5M decommissioning) with no
source documentation, producing CAS50 $\approx$ \$18M---roughly
20$\times$ too low. The Woodruff MIF example reports CAS50 at 10\% of
total capital \citep{woodruff2026}; our D-T estimate of $\sim$8\% is
consistent with this, while our p-${}^{11}$B estimate of $\sim$5\%
correctly reflects reduced spare parts, negligible fuel cost, and
lighter decommissioning obligations.
