...
Each pulse delivers a driver energy $E_{\text{drv}}$ (MJ) at a
repetition rate $f_{\text{rep}}$ (Hz).  The average driver power is
$P_{\text{drv}} = E_{\text{drv}} \, f_{\text{rep}}$.  The driver
wall-plug efficiency $\eta_{\text{pin}}$ relates stored energy to
delivered energy: $E_{\text{stored}} = E_{\text{drv}} / \eta_{\text{pin}}$.
The charged-particle fraction $f_{\text{ch}}$ is derived from the fuel
...
is:
\begin{equation}
P_{\text{th}} = M_n P_n + P_{\text{ash}} + P_{\text{drv}} + P_{\text{pump}}
\end{equation}
where $M_n$ is the neutron energy multiplier.  All electrical output
...
For inductive recovery concepts (e.g.\ Helion-style FRCs), the driver
...
\end{align}
where $P_{\text{ch,net}} = (1 - f_{\text{rad}})\,P_{\text{ash}}$
...
thermal loads (pumps, BOP) are zeroed automatically.
...
E_{\text{stored}} = \frac{P_{\text{net}}}{f_{\text{rep}} \bigl(
  \eta_{\text{eff}}\,(1 + Q_{\text{sci}}\,f_{\text{ch}}) - 1 \bigr)
  \,\eta_{\text{pin}}}
\end{equation}
...
\hspace{1em}22.01.07 & Power supplies & Power-scaled (0.7); \$/J stored basis for pulsed DEC \\
...
Fusion D-T (this model) & 502 & Enhanced industrial (2.5$\times$ CCGT) \\
...
(\$100--250/kW installed, Siemens, ABB) and utility-scale solar inverters
(\$30--50/kW, NREL 2024) bracket the figure: fusion magnet supplies
...
\subsubsection{Pulsed inductive DEC.}
For pulsed MCF/MIF concepts where the capacitor bank is both the driver
and the energy recovery system, the cost scales with stored energy per
pulse on a \$/J basis:
\begin{equation}
...
Rankine: sCO$_2$ turbomachinery is ${\sim}$85\% smaller by volume, but
recuperators add cost, netting ${\sim}$20\% overall reduction
...
heat source temperatures (${\sim}$600$^\circ$C vs.\ 1500$^\circ$C gas
turbine inlet).  The CAS23 coefficient is 19\% above Rankine due to dual
turbomachinery sets (topping plus bottoming) and HRSG.
