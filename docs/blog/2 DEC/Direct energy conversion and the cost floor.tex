...
\title{Direct Energy Conversion for Fusion: Fuel, Confinement, and the Cost Question}
...
\renewcommand{\shorttitle}{DEC and the Cost Floor}

\hypersetup{
pdftitle={Direct Energy Conversion for Fusion: Fuel, Confinement, and the Cost Question},
pdfauthor={Tal Rubin},
pdfkeywords={fusion energy, direct energy conversion, LCOE, venetian blind, pulsed inductive},
}
...
\begin{abstract}
We examine direct energy conversion (DEC) for fusion power plants across three architectures: venetian-blind grids for magnetic mirrors, pulsed inductive recovery for compression-merging concepts, and MHD generators for heat-cycle replacement. Using the \textsc{1costingfe} framework, we show that DEC barely moves the cost floor, which is set by buildings, electrical plant, O\&M, and financing. DEC's leverage instead shows up in the fully costed plant, by reducing the required fusion power. For p-B11, the bremsstrahlung fraction (83\%) keeps the thermal plant as the primary conversion path; for D-He3, pulsed inductive at 85\% efficiency halves the He-3 burn but still costs more than a turbine on hardware. Three concrete development targets emerge: venetian blind sustained \textgreater60\% on real plasma, Helion's round-trip efficiency demonstrated on Polaris, and MHD pressure drop solved at fusion field strengths.
%
%
Fusion power plants have to convert plasma energy into electrical energy. Energy conversion through a thermal cycle is the mature technology, with off-the-shelf components, but with a thermodynamic efficiency limited by the working temperature of the first wall. Direct energy conversion (DEC) is less mature, but allows for direct interaction with the hot plasma, with a near 100\% thermodynamic limit. In this paper we consider the economic viability of two relatively mature DEC schemes as means to reduce the levelized cost of electricity (LCOE) of a fusion power plant. We find that under conservative assumptions, venetian blind has a similar capital cost as the thermal cycle components, and that pulsed inductive DEC requires a substantial learning rate.
%
%
Fusion power plants must convert plasma energy into electricity. The mature path is a thermal cycle, with off-the-shelf components but efficiency capped by first-wall temperature. Direct energy conversion (DEC) is less mature but interacts with the plasma directly, raising the thermodynamic ceiling to nearly 100\%. We use the \textsc{1costingfe} framework to evaluate two relatively mature DEC architectures, venetian blind and pulsed inductive, as candidates for reducing the levelized cost of electricity (LCOE) of a fusion power plant. DEC barely moves the cost floor, which is dominated by buildings, electrical plant, O\&M, and financing. Its leverage shows up in the fully costed plant by reducing the required fusion power: for p-B11, dominant bremsstrahlung keeps the thermal plant as the primary conversion path; for D-He3, pulsed inductive can halve the He-3 burn but still costs more than a turbine on hardware. Three concrete development targets emerge: venetian blind efficiency sustained on real plasma, Helion's round-trip efficiency demonstrated on Polaris, and MHD pressure drop solved at fusion field strengths.
\end{abstract}

\keywords{fusion energy \and direct energy conversion \and LCOE \and venetian blind \and pulsed inductive}
...
grid. These are mature technology with known capital costs.
...
Charged-particle DEC skips both, turning plasma particles directly into
...
neutrons) and makes p-B11 marginal (83-97\% bremsstrahlung). D-He3 is
the natural fit (20-30\% bremsstrahlung, 5\% neutrons).

Different confinements need different capture hardware: venetian-blind
grids for mirrors, inductive coils for pulsed concepts. This paper walks
...
analysis} of the industrial floor on fusion electricity cost (the LCOE
remaining when the fusion core is treated as free); we reference it when
free-core numbers appear, but this piece stands alone.

\section{What Direct Energy Conversion Does}\label{what-direct-energy-conversion-does}

The Carnot limit bounds any heat engine to an efficiency ceiling, η ≤ 1 − T\_cold/T\_hot. In a
...
K cold sink; real cycles lose another 10-15 percentage points to
heat-exchanger and turbine irreversibilities, which is why sCO2 Brayton
sits at 47\% and combined cycle at 53\%. Charged-particle DEC interacts
directly with the plasma, without a solid first wall as an intermediary.
The temperature of the fusion plasma is 10⁸-10⁹ K, which brings the
Carnot limit to essentially 100\%. This gap is why DEC is interesting
in principle. Engineering and material constraints impose
efficiency losses on top of the Carnot limit. Whether it pays off in
practice is the question this paper takes up.
...
Photon emission from a fusion plasma comes from plasma dynamics, not
nuclear physics: electrons radiate bremsstrahlung as they scatter off
ions. At fusion temperatures it becomes a debilitating loss channel. For p-B11, the high plasma temperature (\textgreater100
...
17\% (\href{https://doi.org/10.1103/PhysRevE.106.055215}{Ochs et al.,
2022}), but photons still carry 83\% of the fusion power, and they hit
the walls as heat. A thermal plant is the clear candidate to convert this 
heat to electricity.

\begin{figure}[h]
\centering
\includegraphics[keepaspectratio,width=0.75\linewidth]{energy_channel_distribution.pdf}
\caption{Fractional split of fusion energy by output channel, by fuel
and operating point.}
\end{figure}

\begin{figure}[h]
\centering
\pandocbounded{\includegraphics[keepaspectratio]{p-B11 Thermal Cycle.pdf}}
\caption{Power flow in a 1 GWe p-B11 thermal-only mirror: 83\% of fusion
power (1,990 MW) is re-radiated as bremsstrahlung, 17\% (448 MW) escapes
as charged-particle transport, and both feed a single sCO2 turbine at
47\%.}
\end{figure}

\section{Three Paths to Fusion DEC}\label{three-paths-to-fusion-dec}

\subsection{Venetian Blind}\label{venetian-blind}
...
efficiency} on real end-loss plasma at LLNL's TMX (Barr \& Moir, 1983),
with a
...
\subsection{Pulsed Inductive}\label{pulsed-inductive}
...
\subsection{MHD Generator}\label{mhd-generator}
...
\section{Modeled Approaches}\label{modeled-approaches}
...
dispatch}. The ``floor'' is the LCOE that remains after zeroing out the
fusion core: the irreducible balance-of-plant, buildings, O\&M, and
financing cost a fusion plant carries even if the heat source were free.
Numbers can be reproduced with the
...
\subsection{Venetian Blind + Thermal Hybrid (p-B11)}\label{venetian-blind-thermal-hybrid-p-b11}

We carry the p-B11 mirror baseline forward from the
\href{https://1cf.energy/fusions-cost-floor-what-if-the-core-were-free/}{prior
dispatch} and add a venetian blind as an overlay. The thermal cycle is
kept because 83\% of p-B11 energy radiates as bremsstrahlung; the DEC
captures only the charged particles leaving the mirror axially.

Sizing the VB hardware to 400 MWe DEC output (2024 NOAK) lands an all-in
figure of roughly \$140M, range \$105-190M: a 304L stainless tank
(\$8-12/kg fabricated, 1500-3000 tonnes, \textasciitilde\$20M), HV power
conditioning at \$100-120/kW (60-70\% of a full
...
VB DEC at 48\% + thermal (hybrid) & 19 & 1,266 \\
VB DEC at 60\% + thermal (hybrid) & 19 & 1,261 \\
\bottomrule
\end{tabular}
\end{table}

The venetian blind doesn't lower the floor. With 83\% of fusion energy
radiated as bremsstrahlung, the DEC captures 90\% of the 17\%
...
\$19/MWh, \$1/MWh above thermal-only at \$18/MWh, and adds about
\$40-45/kW on overnight capital. Eliminating the turbine entirely and wasting the
...
The numbers above use the mirror baseline heating power of 40 MW, giving
Q\_eng around 8 and a 13\% recirculating fraction. If the heating power
is larger, the picture shifts. The heating power leaves the plasma in
the charged-particle transport channel, and is mostly captured by the
VB. But at a heating wall-plug efficiency of 50\% (the model's NBI
default) the injection chain draws 2 MW of gross electric per 1 MW
delivered to the plasma, while the transport channel returns only 0.58
MW in the VB 60\% hybrid. Each MW of heating therefore costs roughly 1.4
MW of gross electric output (2 MW drawn in, 0.58 MW returned), and the
fusion core upsizes to cover the shortfall. Holding net output at 1 GWe,
LCOE rises by \$2/MWh at 150 MW heating, by \$3/MWh at 250 MW, and by
\$6/MWh at 400 MW; beyond 600 MW Q\_eng drops below 2 and the
recirculating fraction exceeds 50\%. Thermal-only stays cheaper than the
thermal+VB hybrid by about \$0.9/MWh at every heating level: the VB
hardware cost scales with DEC throughput alongside the recovery it
provides, so the gap stays roughly flat across the sweep.

Flipping the analysis: at fixed VB hardware cost, what efficiency would
justify adding the DEC? At the 1 GWe p-B11 baseline the VB outputs
roughly 232 MWe (90\% capture of 429 MW charged-particle margin at
60\% conversion), and the model scales the VB hardware to about \$96M at
this plant scale. Holding that fixed, no VB efficiency between 30\% and
100\% breaks even with thermal-only; the VB hardware exceeds
turbine-side savings across the entire physically accessible band.
Break-even enters the physically accessible window only if the VB capex
at this plant scale falls by roughly 3x, to around \$30M, at which point
a 54\% VB suffices. The
leverage is narrow: only 15\% of fusion power (0.9 x 0.17) flows through
the DEC, so each percentage point of VB efficiency moves only 0.12
percentage points of plant-level conversion.

\subsection{Pulsed Inductive (D-He3)}\label{pulsed-inductive-d-he3}
...
inverter at the back. Sizing the pulsed chain downstream of the driver
to 1 GWe net (NOAK) gives an all-in figure of roughly \$593/kW: a
recovery cap bank sized to the fusion power balance (703 MJ at
\$0.50/J = \$351M), DC-DC links at \$75/kW of gross output (\$78M), a
\href{https://cdn.misoenergy.org/20240501\%20PSC\%20Item\%2004\%20MISO\%20Transmission\%20Cost\%20Estimation\%20Guide\%20for\%20MTEP24632680.pdf}{VSC
HVDC valve hall + IGBT grid-tie} at \$150/kW net (\$150M), and controls
at 4\% of the driver bank (\$14M). The grid-tie line is set by
industrial HVDC pricing rather than commodity solar-inverter pricing
(\href{https://docs.nrel.gov/docs/fy25osti/92536.pdf}{\$30-50/kW NREL
benchmark}), reflecting pulse-rated topology and high-voltage
interconnect. The compression bank itself (\$351M) is accounted for as
...
\begin{figure}[h]
\centering
\pandocbounded{\includegraphics[keepaspectratio]{D-He3 Pulsed Inductive DEC.pdf}}
\caption{Power flow in a 1 GWe D-He3 mirror with pulsed inductive DEC at
85\%: 95\% of charged-particle transport (1,062 MW of 1,118 MW) drives
the DEC, while bremsstrahlung (25\% of fusion power) and neutrons drive
a thermal bottoming cycle.}
\end{figure}

\section{What the Floor Is, and Isn't}\label{what-the-floor-is-and-isnt}
...
\midrule
Thermal only (sCO2 Brayton, 47\%) & 7.6 & 821 & +\$2.4/MWh \\
VB DEC 60\% + thermal (hybrid) & 7.9 & 858 & +\$2.1/MWh \\
\bottomrule
\end{tabular}
\end{table}
...
because wasting 83\% of fusion energy as bremsstrahlung heat requires an
...
\section{Where DEC Pays Off}\label{where-dec-pays-off}
...
blanket systems. For pulsed inductive, the core jumps \$381M because the
pulsed-power chain that replaces the turbine (dominated by the recovery
cap bank) is substantial. LCOE drops \$10/MWh with the venetian blind
and \$25/MWh with pulsed inductive, but the pulsed savings come entirely
from fuel, not hardware: higher efficiency means less fusion power per
MWh, which means less He-3 burned. For D-He3, DEC is primarily a
fuel-saving technology.

For p-B11 (negligible fuel cost), the effect is smaller: thermal LCOE is
\$37/MWh, VB DEC 60\% is \$37/MWh (core \$1,208M vs \$1,245M). Because
bremsstrahlung dominates, the DEC captures only the 17\%
...
Capacitor lifetime is a separate risk: the model assumes 10⁸ shot
lifetime (NOAK), but current high-energy-density capacitors achieve
roughly 10⁷ cycles. At a 1 Hz rep rate, 10⁷ shots is four months
of operation, requiring frequent bank replacements that add both capital
cost and downtime.

\section{The Bremsstrahlung Constraint}\label{the-bremsstrahlung-constraint}
...
bremsstrahlung is 83\% of fusion power. Without driving the plasma out
...
\begin{enumerate}
\item \textbf{Thermal cycle}: a full turbine island converts bremsstrahlung
  heat at 47--53\%. This is what the ``hybrid'' configurations above
  assume. It works but preserves essentially the full thermal BOP.
\item \textbf{MHD on the coolant}: if the bremsstrahlung-heated wall coolant
  is liquid metal, an MHD channel could convert at \textgreater60\% with
  no turbine. Undemonstrated at reactor scale.
\item \textbf{Reject as waste}: accept the efficiency loss and dump the
  bremsstrahlung heat to cooling towers. At 83\% bremsstrahlung
  fraction, this means wasting most of the fusion energy. The overall
  plant efficiency collapses: a DEC at 85\% efficiency on the 17\% net
  charged fraction yields only 14\% of fusion power as electricity, far
  worse than thermal-only (47\%).
\item \textbf{X-ray photovoltaic DEC}: convert bremsstrahlung directly to
...
Synchrotron is the other photon channel, and it does not impose a
comparable constraint. The fundamental electron cyclotron frequency at
10 T is 280 GHz (1.2 × 10⁻³ eV), with harmonics extending into the THz
band at fusion electron temperatures, six orders of magnitude below
bremsstrahlung. Mm-wave synchrotron reflects at \textgreater99\% from
polished metallic first walls and is typically recycled into the plasma
rather than captured; the few-percent escaping fraction could in
principle feed a rectenna using mature GHz-band hardware, but the upside
is bounded.

\begin{figure}[h]
\centering
\pandocbounded{\includegraphics[keepaspectratio]{p-B11 VB DEC.pdf}}
\caption{Power flow in a 1 GWe p-B11 mirror with a venetian-blind DEC at
60\% hybrid with a thermal cycle: 83\% of fusion power (1,901 MW)
radiates as bremsstrahlung and feeds the turbine, while the DEC captures
only the 17\% charged-particle margin (386 MW).}
\end{figure}

\section{Learning Rates}\label{learning-rates}
...
The pulsed floors in this paper already assume the 40--100x reduction has
...
\begin{figure}[h]
\centering
\pandocbounded{\includegraphics[keepaspectratio]{efficiency-cost chart.pdf}}
\caption{Efficiency and capital cost ranges for the four conversion
architectures. MHD is dashed because no fusion-relevant cost basis
exists; pulsed inductive assumes the \$0.50/J NOAK capacitor target.}
\end{figure}
...
baseline conditions, the p-B11 cost floor is \$18/MWh, and a venetian
blind DEC overlay raises it slightly rather than lowering it. With 83\% of fusion energy
...
to \$7.6-7.9/MWh. The floor is dominated by buildings, electrical
...
p-B11 radiates 83-97\% as bremsstrahlung. Thermonuclear operation leaves
a 3\% margin; driving the plasma out of equilibrium widens it to 17\%
...
\begin{enumerate}
\item Moir, R.W. \& Barr, W.L., ``Venetian-blind direct energy converter for
  fusion reactors,'' \emph{Nuclear Fusion} 13, 35--45 (1973).
  \href{https://iopscience.iop.org/article/10.1088/0029-5515/13/1/005}{Link}
\item Hoffman, M.A., ``Electrostatic Direct Energy Converter Performance and
  Cost Scaling Laws,'' UCID-17560, LLNL (1977).
  \href{https://www.osti.gov/biblio/7218298}{Link}
\item Slough, J. et al., ``Creation of a high-temperature plasma through
...
\item Putvinski, S.V., Ryutov, D.D. \& Yushmanov, P.N., ``Fusion reactivity
  of the p-B11 plasma revisited,'' \emph{Nuclear Fusion} 59(7), 076018
  (2019). \href{https://doi.org/10.1088/1741-4326/ab1a60}{Link}
\item Ochs, I.E. et al., ``Improving the feasibility of economical
...
\item Kirtley, D. et al., ``Generalized burn cycle efficiency framework,''
  APS DPP 2024, Abstract GO05.8.
  \href{https://youtu.be/5nHmqk1cI2E?t=505}{Presentation}
\item CATF IWG, ``Extension of the Fusion Power Plant Costing Standard,''
  arXiv:2602.19389 (2026). \href{https://arxiv.org/abs/2602.19389}{Link}
\item Rosa, R.J., \emph{Magnetohydrodynamic Energy Conversion}, Hemisphere
  Publishing (1987).
  \href{https://inis.iaea.org/records/4ss3z-rqd83}{Link}
\item GAO, ``Magnetohydrodynamics: A Promising Technology for Efficiently
  Generating Electricity From Coal,'' EMD-80-14 (1980).
  \href{https://www.gao.gov/products/emd-80-14}{Link}
\item Shea, D.A. \& Morgan, D., ``The Helium-3 Shortage: Supply, Demand, and
...
\item Barr, W.L. \& Moir, R.W., ``Test Results on Plasma Direct
  Converters,'' \emph{Fusion Technology} 3(1), 98-111 (1983).
  \href{https://doi.org/10.13182/FST83-A20820}{Link}
\item Fabris, G. \& Hantman, R.G., ``Interaction of fluid dynamics phenomena
