...
1\,GWe, falling to \$9.6/MWh only at an extreme limit of 5\,GWe
with maximally favorable financing; a proton-boron-11 plant
attains \$17/MWh at baseline and \$5.0/MWh at the same extreme
limit, leaving meaningful budget for the fusion core itself. Two
...
carries 83\% or more of the fusion power and constrains DEC to capturing only
...
Published cost estimates for commercial fusion power plants vary
widely. A recent compilation of first-of-a-kind capital expenditure
projections spans more than an order of magnitude, from \$1{,}400
to \$43{,}000 per kilowatt installed \citep{Tang2026}. Levelized
cost of electricity (LCOE) projections vary by roughly a factor of
three: optimistic n-th-of-a-kind advanced tokamaks have been
projected at \$47/MWh \citep{Najmabadi2006}; first-generation
EU-DEMO scenarios reach \$160/MWh and above \citep{Entler2018};
most serious estimates fall between \$50 and \$130/MWh
\citep{SheffieldMilora2016, Lindley2023}. The variability reflects genuine
...
This paper examines the minimum cost of electricity from a fusion power plant, 
working with the thesis that an electricity cost of one cent per 
kilowatt-hour (\$10/MWh) would be transformative, and would more than justify the
investment in fusion development. The analysis proceeds in two parts.
Section~\ref{sec:floor} characterizes the cost floor, the cost of buildings,
balance-of-plant equipment, indirect
...
specific core technology, which is assumed to be free in the floor analysis.
Section~\ref{sec:dec} examines the choice
...
a fuel-dependent component. The fusion core itself (top block) is
assigned zero capital cost in the floor analysis of
Section~\ref{sec:floor}; the fuel-dependent block immediately below
it (shielding, tritium system, radioactivity handling, nuclear
HVAC) is retained because those costs are imposed by the fuel
choice, not by the core technology.}
\label{fig:plant}
\end{figure}
...
cycles the thermal cycle is the only power conversion pathway
considered in this floor analysis. The BOP equipment subtotal is
essentially fuel-invariant when DEC is excluded (\$386M for both
p-B11 and D-He3, compared with \$393M for D-T), because turbine,
heat-rejection, and electrical hardware all scale with
thermal-electric power rather than with fuel choice. DEC
architectures are evaluated separately in Section~\ref{sec:dec}
...
cryogenics facility, switchgear building, assembly hall) are common to all
...
1\,GWe consequently range from \$359M for p-B11 to \$574M for D-T
(Table~\ref{tab:fuels}).
...
management, 20\% of direct costs), owner's costs,
...
\$9.6/MWh only at the extreme limit: 5\,GWe net
...
The constraint is structural. D-T buildings cost \$574M because
...
monitoring; buildings amount to \$417M and the BOP-only floor at
...
yielding \$359M at 1\,GWe and a BOP-only floor of \$17/MWh.
...
The \$215M building cost gap between D-T and p-B11 at 1\,GWe is
...
\$7.1/MWh, leaving \$2.9/MWh for the fusion core (approximately
\$1{,}145/kW of overnight capital). This figure sits squarely
inside the overnight cost of a natural gas combined-cycle plant
(\$900--1{,}200/kW), with the difference that the fusion core
lacks the deployment scale and manufacturing maturity of
combined-cycle hardware. At the extreme limit (5\,GWe, 95\%
availability, 2\% WACC, 50-year life, 3-year construction), the
p-B11 floor falls to \$5.0/MWh.
...
(see Section~\ref{sec:dt}) is shown at plant scales of 2\,GWe and
3\,GWe.}
...
lower the 2\,GWe aggressive p-B11 floor from \$7.1/MWh to
approximately \$4.2/MWh, widening the available budget for the
costed core to \$5.8/MWh.
...
costed core.
\item Fuel choice exerts a buildings-and-staffing lever larger than
most published core cost-reduction scenarios; BOP equipment is
essentially fuel-invariant when DEC is excluded (\$386--393M
across all three fuels), but aneutronic fuel downgrades the
buildings from enhanced-industrial to industrial and removes the
tritium infrastructure and radiation-specific staffing entirely.
...
D-T deposits 20\% of its fusion energy in 3.5\,MeV alpha particles,
a charged fraction that nominally exceeds the 17\% non-equilibrium
p-B11 margin. From a DEC perspective, however, D-T resembles p-B11:
the bulk of the energy (80\% carried by 14.1\,MeV neutrons rather
than bremsstrahlung photons) requires a full thermal plant, so DEC
acts at most as a small bottoming overlay on the alpha channel. D-T
also carries the radioactivity burden absent from p-B11: neutron
activation of structural materials, lithium breeding blankets, and
tritium inventory handling all increase plant cost without altering
the DEC arithmetic. D-T is therefore treated as a thermal fuel in
this analysis and is not considered further in this section.
...
\subsection{p-B11's Bremsstrahlung Constraint}
\label{sec:brem}
...
above 60\% but no fusion-scale demonstration \citep{Smolentsev2022}
(discussed further in Section~\ref{sec:mhd});
...
\midrule
Thermal only (sCO\textsubscript{2} Brayton, 47\%) & 17 & 1{,}221 \\
...
the hybrid floor is \$18/MWh, \$1/MWh above thermal-only, with
...
(\$/MWh) \\
\midrule
Thermal only (sCO\textsubscript{2} Brayton, 47\%) & 18 & 74 & 92 \\
...
cost in the thermal plant; buildings (\$417M), the electrical
plant (\$96M), the operations and maintenance budget
(\$31M per year), and financing charges together exceed it.
...
\subsubsection{Magnetohydrodynamic Conversion}
\label{sec:mhd}

A third conversion path, not modeled in the framework above but
worth noting, is the magnetohydrodynamic (MHD) generator: a
transverse magnetic field drives current through electrodes via the
Lorentz force on a conducting fluid, with no moving parts. MHD is
not a charged-particle DEC: it replaces the Brayton turbine, not
the heat cycle, and remains Carnot-limited. For aneutronic fuels it
could handle the bremsstrahlung fraction that charged-particle DEC
cannot touch. Coal-combustion MHD reached TRL 5--6 through national
programs in the 1960s--1990s; liquid-metal MHD (LMMHD) has
separately shown $>$71\% efficiency at small scale
\citep{Fabris1981}, and projected efficiency for LMMHD on fusion
coolants exceeds 60\% \citep{Smolentsev2022}.

Fusion MHD has a costed paper-design heritage. The Compact Fusion
Advanced Rankine (CFAR) concept developed in the late 1980s by an
LLNL/Kyoto/TIT collaboration
\citep{HoffmanCampbellLogan1989,Logan1993} pairs a high-temperature
mercury Rankine loop and microwave superheater with a
nonequilibrium MHD disk generator, packaged to fit almost entirely
inside the reactor vault. The magnetically-confined variant reported
24--30\% net plant efficiency at 2500--3250\,K generator-inlet
temperature; the inertial CFAR-II variant, with a vaporized blanket
plasma at $10^4$\,K, projected 38\%. Both require bespoke blankets
at or past current materials limits, and neither was built. CFAR-II
carries a costed 22-subsystem BOP model with direct cost of
30--40\,M\$/GWe in 1992 dollars, the most explicit fusion-MHD BOP
cost reference on record, although that figure excludes the blanket
development gap and the projected efficiencies sit below the
$>$60\% modern LMMHD target.

Folded into the framework of this paper, in-vault integration could
trim approximately \$200M of BOP and buildings capital expenditure
at 1\,GWe. CFAR-II's 30--40\,M\$/GWe BOP cost inflates to roughly
\$70--90M in 2024 dollars, against the \$168M sCO\textsubscript{2}
turbine island it would replace; the net conversion-swap saving is
\$80--100M, with the balance from turbine-hall and high-temperature
piping elimination, yielding a total LCOE saving of approximately
\$3/MWh under the assumption that blanket costs are unchanged.

This may understate CFAR's intended impact. The original motivation
was that a fusion core paired with a conventional BOP cannot reach
the cost per MWh of fission, gas, or coal because the core is
intrinsically expensive, so drastically shrinking the BOP is the
path to competitiveness, not a marginal optimization on an
already-competitive plant.

The remaining obstacle is that no fusion-MHD system has ever been
built. The electrode-erosion problem that ended the coal programs
does not apply to clean fusion coolants. For LMMHD integrated into
a breeding blanket \citep{Smolentsev2022}, the pressure drop of
liquid metal flowing through fusion-scale magnetic fields
(5--15\,T) may exceed the generated power and remains unresolved at
reactor scale. CFAR avoids this by topology: the disk generator
sits outside the toroidal field coils in a region where the field
has dropped off rapidly, with dedicated ring magnets, and the 1989
design study \citep{HoffmanCampbellLogan1989} reported cycle
efficiency insensitive to blanket pressure drops up to 50\%. MHD
therefore remains a what-if at the hardware level (no operational
...
LCOE from \$119/MWh to \$109/MWh, with a modest \$22M
increase in core cost partially offset by reduced heating-system
capacity. The pulsed inductive scheme lowers LCOE to \$81/MWh,
...
required fusion power. Thermal LCOE is \$42/MWh; the
venetian-blind DEC at 60\% yields \$42/MWh as well, with core
costs of \$1{,}553M and \$1{,}585M respectively. The DEC overlay
...
energy stored (all-in: capacitors, switches, charging, buswork)
