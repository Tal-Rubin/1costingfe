...
independent parameters. It spans the several confinement families
across magnetic, inertial, and magneto-inertial confinement schemes.
\textsc{1costingfe} is developed to assist in the identification 
...
baseload generation at scale, so \textsc{1costingfe} defaults represent 
...
$\eta_{\text{th}}$, and (where applicable) $\eta_{\text{de}}$,
...
$T_e$         & 70 keV              & per Section~3.1.3 example \\
...
group-level cost rollup is shown in \cref{tab:tour-rollup}.
...
CAS60                  & 572.0 \\
\midrule
Total overnight        & 3547.8 \\
Overnight specific cost (\$/kW) & 3548 \\
\midrule
CAS70 (M\$/yr)         & 34.5 \\
CAS80 (M\$/yr)         & 438.4 \\
CAS90 (M\$/yr)         & 285.9 \\
\midrule
\textbf{LCOE (\$/MWh)} & \textbf{99.6} \\
\bottomrule
\end{tabular}
\end{table}
...
physics-output parameter is \texttt{eta\_de} (elasticity $-0.49$),
the most consequential cost unit price is \texttt{fuel\_recovery}
($-5.35$), and the most consequential financial parameter is
\texttt{interest\_rate} ($+0.28$). The \texttt{fuel\_recovery} entry
dominates because unrecovered helium-3 is both expensive and directly
proportional to the fuel cycle cost component of LCOE. The financial
bucket entries are modest for this reference scenario, reflecting the
relatively low capital intensity of mirror designs: \texttt{interest\_rate}
carries an elasticity of $+0.28$ and \texttt{inflation\_rate} $+0.21$.
...
\midrule
\texttt{fuel\_recovery}  & $-5.347964$ & $-5.347968$ & $8.89{\times}10^{-7}$ \\
\texttt{u\_he3}          & $+0.577189$ & $+0.577207$ & $3.12{\times}10^{-5}$ \\
\texttt{eta\_de}         & $-0.487345$ & $-0.487513$ & $3.45{\times}10^{-4}$ \\
\texttt{availability}    & $-0.417998$ & $-0.417972$ & $6.19{\times}10^{-5}$ \\
\texttt{burn\_fraction}  & $-0.296286$ & $-0.296324$ & $1.28{\times}10^{-4}$ \\
\bottomrule
\end{tabular}
\end{table}
...
Explicit values passed by the user override this default.
...
stellarator: $4\pi^2 \times f_{\text{path}}$; mirror:
$n_{\text{coils}} \cdot 4\pi$, where $n_{\text{coils}}$ is the
number of independent solenoid coils in the mirror layout),
...
The mirror $n_{\text{coils}}$ default is 10, calibrated to a Realta
HAMMIR-class tandem mirror: four end-plug HTS coils (two per end, in
the Hammer evolution) plus six LTS solenoids discretizing the 50~m
central cell at conservative pitch. Simple-mirror devices
(WHAM/BEAM/Anvil) use $n_{\text{coils}} \approx 4$.
...
Mirror      &  2.5$\times$ & 1.0 & \$513M & 10 simple solenoids (HAMMIR-class) \\
...
expected, and the question is whether the divergences land in the
...
are summarised in \cref{tab:arc-inputs}.
...
The published fabricated-component cost is \$5,560\,M (FY2014~USD,
\citep{sorbom2015arc} p.~25, Section 6.6), or \$7,562\,M after
escalation to 2025~USD. \textsc{1costingfe} predicts an overnight
capital cost of \$3,224\,M, an overnight specific cost of
\$11,940\,/kWe, and an LCOE of \$160.2\,/MWh under the framework's
NOAK default assumptions. The implied delta on overnight cost is
$-57$\,\%; \textbf{TODO: discussion paragraph -- Sorbom's volumetric
estimate uses a single \$1.06\,M/tonne fabricated multiplier averaged
across four heritage burning-plasma designs (FIRE, BPX, PCAST5,
ARIES-RS), with the dominant contribution from a 4{,}350-tonne
SS316LN structural support priced at \$4.6\,B. \textsc{1costingfe}
replaces this with bottom-up CAS21 (buildings, fuel-cycle-dependent
scoping) and CAS22 (procurement-grounded magnet, vessel, and shielding
costs); the gap is consistent with replacing a heritage burning-plasma
multiplier with current procurement and reflects a NOAK assumption
not present in Sorbom's estimate.}
...
(escalated to 2025~USD) with \textsc{1costingfe}'s prediction. The
...
% Direct: published 1521.1 M$ (1992) * 2.295 = 3491; 1cfe sum CAS21+22+23+24+25+26+27 = 3683
% Overnight $/kWe: published 2441 (1992) * 2.295 = 5602; 1cfe (CAS10-50)/kWe = 4822
% Total: published 2844 M$ (1992) * 2.295 = 6527; 1cfe total_capital = 5749
% LCOE: published 47.53 mill/kWh (1992) * 2.295 = 109.1 $/MWh; 1cfe = 80.4
\begin{table}[h]
\centering
...
\begin{tabular}{lrrr}
\toprule
Account & Published (2025~USD) & \textsc{1costingfe} & $\Delta$ (\%) \\
\midrule
Direct cost (Account 90), M\$       & 3{,}491 & 3{,}683 &  $+5.5$ \\
Overnight specific cost, \$/kWe     & 5{,}602 & 4{,}822 & $-13.9$ \\
Total cost (Account 99), M\$        & 6{,}527 & 5{,}749 & $-11.9$ \\
LCOE, \$/MWh                        &   109.1 &    80.4 & $-26.3$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{TODO: ARIES-AT discussion paragraph -- the direct-cost row
agrees within 6\,\%, indicating that the bottom-up CAS21--27
procurement basis lands close to the heritage 2002 ARIES systems-study
estimate at the top level. The overnight specific cost is 14\,\%
lower; this is consistent with \textsc{1costingfe}'s NOAK default
assumption replacing the FOAK-with-learning-multiplier convention used
in the ARIES program. The LCOE divergence (-26\,\%) is dominated by
the same NOAK effect propagated through the capital recovery factor.}
...
\textbf{TODO: LCOE composition discussion paragraph -- ARC and
ARIES-AT under \textsc{1costingfe} produce broadly similar capital
stacks dominated by CAS22 (reactor plant equipment), with CAS21
(buildings) the second-largest group in both cases. The most
informative compositional difference is the absolute scale: ARC's
compact 270\,MWe footprint produces a smaller absolute capital stack
than the 1\,GWe ARIES-AT but a higher \$/kWe figure, reflecting
diseconomies of scale in CAS21 buildings and CAS30 indirect services
that do not shrink linearly with plant size. The
$\eta_\text{th}=0.40$ vs.\ 0.59 thermal-cycle gap shifts gross-electric
recirculation but does not materially shift the capital-stack
composition shown here, since \cref{fig:benchmark-stacks} reports
capital cost rather than LCOE per MWh.}
...
results in \cref{sec:benchmark} are reproduced by the scripts
