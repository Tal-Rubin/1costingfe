...
Cost accounts follow the Code of Accounts Structure (CAS), originated by Schulte et~al.\
at PNL in 1978, adapted for fusion by the ARIES program, and later formalized by the
Generation~IV Economic Modeling Working Group.
The framework spans the four candidate fuel cycles (D-T, D-D, D-${}^3$He, p-${}^{11}$B)
and the principal confinement families (tokamak, stellarator, mirror, field-reversed
configuration, laser and Z-pinch inertial systems, and magneto-inertial concepts),
with each cost account justified independently from procurement and benchmark data
rather than from heritage scaling factors alone.
The use of JAX as the computational backend exposes exact gradients of LCOE with
respect to every input parameter through reverse-mode automatic differentiation,
and supports vectorized sensitivity sweeps and Monte Carlo uncertainty propagation
via \texttt{vmap}, transforming what are traditionally bottleneck operations into
routine ones.
The purpose of \textsc{1costingfe} is to enable identification of technological
corridors compatible with a 1~cent/kWh LCOE target.
We describe the economics module, the physics module, and the cost account
methodology, noting where the treatment diverges from prior work.
\end{abstract}
...
are planned but not yet exercised in this paper. Cross-validation of
LCOE outputs against ARIES and pyFECONS reference cases is in progress.
