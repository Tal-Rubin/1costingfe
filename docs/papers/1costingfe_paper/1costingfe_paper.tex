...
conventions. The framework is implemented in JAX with reverse-mode
automatic differentiation, exposing exact LCOE gradients for
sensitivity analysis and Monte Carlo uncertainty propagation. A
worked tour from physics outputs to LCOE is included as a use case,
...
\begin{enumerate}
\item Fusion physics remains uncertain at the plant level: no
  combination of confinement approach (magnetic, inertial,
  magneto-inertial), system (tokamak, stellarator, laser-driven, etc.)
  and fuel cycle (D-T, D-D, D-${}^3$He, or p-${}^{11}$B) has yet
  demonstrated sustained net electricity generation, and the
  configuration that will prove commercially viable is not yet known.
\item Plant designs are pre-conceptual: physics models, engineering
  geometry, manufacturing costs, and procurement data are all
  incompletely specified, so the subsystem and material inputs to
  the cost accounts carry inherent uncertainty.
\end{enumerate}

To pursue this search systematically, \texttt{1costingfe} is under active development as a
...
\texttt{1costingfe} is the cost-accounting layer of a broader fusion
...
The worked tour in \cref{sec:tour} presents a representative
...
\begin{table}[htbp]
\caption{Physics outputs handed to \texttt{1costingfe} for the
...
The tornado identifies, in a single figure, the parameters whose uncertainty
dominates LCOE: those with long bars indicate where simulation effort or
vendor data reduces cost uncertainty most efficiently. For the reference scenario, the most consequential
...
\midrule
D-T    & 626 & 626 & Enhanced industrial (tritium confinement, hot cell, rad-HVAC) \\
D-D    & 573 & 573 & Enhanced industrial (reduced tritium/activation scope) \\
D-${}^3$He & 450 & 450 & Light enhanced industrial \\
p-${}^{11}$B & 384 & 384 & Industrial (no hot cell, no tritium, standard HVAC) \\
\bottomrule
\end{tabular}
\end{table}
...
twin integration; the figure may be varied via
...
$-57$\,\%. Sorbom's volumetric estimate uses a single \$1.06\,M/tonne
...
The direct-cost row agrees within 6\,\%, indicating that the
bottom-up CAS21--27 procurement basis lands close to the heritage 2002
ARIES systems-study estimate at the top level. The overnight specific
cost is 14\,\% lower; this is consistent with \texttt{1costingfe}'s
NOAK default assumption replacing the FOAK-with-learning-multiplier
convention used in the ARIES program. The LCOE divergence ($-26$\,\%)
...
by CAS22 (reactor plant equipment), with CAS21 (buildings) the
second-largest group in both cases. The most informative compositional
...
Here $\eta_{\text{pin}}$ characterises pulsed-driver hardware (laser, ion
