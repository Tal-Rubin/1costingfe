...
\texttt{interest\_rate} ($+0.38$). The unusually large
\texttt{fuel\_recovery} elasticity reflects proximity to a physical
limit: at the NOAK default of 0.99 the residual loss fraction
$(1 - f_r) = 0.01$ is small in absolute terms, so a one-percentage-point
absolute improvement halves it, and for a D-${}^3$He plant where the
helium-3 market price is itself a major cost-side lever this propagates
strongly to LCOE. The financial bucket entries are modest for this
...
\hspace{1em}22.01.06 & Vacuum system & Inherited & Volume $\times$ power scale \\
...
27$^\dagger$ & Special materials & Extended & Fuel-dependent blanket fill inventory \\
...
design) are supported by the framework but are not fully demonstrated
in the present text.
